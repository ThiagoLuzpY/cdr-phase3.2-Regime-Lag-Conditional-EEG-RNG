...
def _estimate_epsilon_grid_local(
    ...
    *,
    ...
    if label:
        print(f"[Phase3-EEG] Estimating epsilon on {label} (grid={n_grid})...")
    ...
    if label:
        print(f"[Phase3-EEG] Done epsilon({label}): eps_hat={eps_hat:.4f}")
...
def _shuffle_next_states(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Strong null control:
    destroys temporal pairing between consecutive EEG states.
    """
...
def _circular_shift_next_states(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Preserves marginal structure of next states while breaking the
    correct temporal alignment between current and next EEG states.
    """
    rng = np.random.default_rng(seed)
...
def _block_shuffle_pairs(curr: np.ndarray, nxt: np.ndarray, seed: int, block_size: int):
    """
    Shuffle short temporal EEG blocks.
...
def run_eeg_controls(
    ...
    """
    Run EEG-specific negative controls and estimate epsilon on each.

    Parameters
    ----------
    curr, nxt
        Arrays of current and next encoded EEG states.
    kernel
    ...
    for i in range(cfg.n_controls):
        ...
        if method == "shuffle_next":
            ...
            c_ctrl, n_ctrl = _block_shuffle_pairs(
                ...
                nxt,
                seed=seed,
                block_size=cfg.eeg_control_block_size,
            )
