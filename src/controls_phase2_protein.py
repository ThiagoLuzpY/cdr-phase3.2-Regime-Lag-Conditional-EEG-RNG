...
def _estimate_epsilon_grid_local(
    ...
    *,
    ...
    if label:
        print(f"[Phase2-Protein] Estimating epsilon on {label} (grid={n_grid})...")
    ...
    if label:
        print(f"[Phase2-Protein] Done epsilon({label}): eps_hat={eps_hat:.4f}")
...
def _shuffle_next_states(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Strong null control:
    destroys correct temporal pairing between current and next states.
    """
...
def _circular_shift_next_states(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Preserve next-state marginal structure while breaking correct frame-to-frame pairing.
    """
    rng = np.random.default_rng(seed)
...
def _block_shuffle_pairs(curr: np.ndarray, nxt: np.ndarray, seed: int, block_size: int = 256):
    """
    Preserve short-range local conformational transitions while destroying
    the correct longer-range temporal ordering.
    """
    rng = np.random.default_rng(seed)
...
def run_protein_controls(
    ...
    """
    Run protein-specific negative controls and estimate epsilon on each.

    Parameters
    ----------
    curr, nxt
        Arrays of current and next encoded protein states.
    kernel
    ...
    for i in range(cfg.n_controls):
        ...
        if method == "shuffle_next":
            ...
            c_ctrl, n_ctrl = _block_shuffle_pairs(curr, nxt, seed=seed, block_size=256)
