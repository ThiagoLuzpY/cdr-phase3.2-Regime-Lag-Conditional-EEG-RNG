...
def _estimate_epsilon_grid_local(
    ...
    *,
    ...
    if label:
        print(f"[Phase3-Joint] Estimating epsilon on {label} (grid={n_grid})...")
    ...
    if label:
        print(f"[Phase3-Joint] Done epsilon({label}): eps_hat={eps_hat:.4f}")
...
def _shuffle_next_states(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Strong null control:
    destroys temporal pairing between consecutive joint states.
    """
...
def _circular_shift_next_states(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Preserve next-state marginals while breaking correct alignment.
    """
    rng = np.random.default_rng(seed)
...
def _block_shuffle_pairs(curr: np.ndarray, nxt: np.ndarray, seed: int, block_size: int):
    """
    Shuffle short temporal blocks of joint transitions.
...
def _swap_rng_subspace(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Control aimed specifically at the RNG side of the joint system.

    Assumes joint state is encoded with EEG components first and RNG components last.
    Since this control operates on encoded state ids only, we approximate the effect
    by shuffling next states while preserving current states. This is the cleanest
    surrogate available at this stage without requiring component-level state decoding
    inside controls.
    """
    return _shuffle_next_states(curr, nxt, seed=seed)


def _swap_eeg_subspace(curr: np.ndarray, nxt: np.ndarray, seed: int):
    """
    Control aimed specifically at the EEG side of the joint system.

    As with RNG-side controls, we apply a principled temporal misalignment surrogate
    at the transition level.
    """
    return _circular_shift_next_states(curr, nxt, seed=seed)


def run_joint_controls(
    curr: np.ndarray,
    nxt: np.ndarray,
    kernel,
    cfg,
    ...
    """
    Run joint-domain negative controls and estimate epsilon on each.

    Parameters
    ----------
    curr, nxt
        Arrays of current and next encoded JOINT states.
    kernel
    ...
    for i in range(cfg.n_controls):
        ...
        if method == "shuffle_next":
            ...
            c_ctrl, n_ctrl = _block_shuffle_pairs(
                ...
                nxt,
                seed=seed,
                block_size=cfg.joint_control_block_size,
            )

        elif method == "shuffle_rng_only":
            if getattr(cfg, "joint_control_shuffle_rng_only", True):
                c_ctrl, n_ctrl = _swap_rng_subspace(curr, nxt, seed=seed)
            else:
                c_ctrl, n_ctrl = _shuffle_next_states(curr, nxt, seed=seed)

        elif method == "shuffle_eeg_only":
            if getattr(cfg, "joint_control_shuffle_eeg_only", True):
                c_ctrl, n_ctrl = _swap_eeg_subspace(curr, nxt, seed=seed)
            else:
                c_ctrl, n_ctrl = _circular_shift_next_states(curr, nxt, seed=seed)
