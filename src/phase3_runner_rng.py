...
from config.phase3_config_rng import load_phase3_rng_config
...
from src.controls_phase3_rng import run_rng_controls
...
def _transition_indices_from_epoch_split(
    ...
    """
    Convert an epoch/window split into transition indices without leakage.

    Transition k corresponds to epoch/window pair (k -> k+1).
    ...
    """
    if n_epochs < 2:
        raise RuntimeError(f"Need at least 2 epochs/windows, got {n_epochs}")
...
# ---------------------------------------------------------
# Phase III RNG Runner
# ---------------------------------------------------------

def run_phase3_rng():
    cfg = load_phase3_rng_config()
    cfg.ensure_paths()

    print("\n==============================")
    print("CDR Phase III — RNG Domain")
    print("==============================\n")

    print("[Phase3-RNG] Configuration loaded")
    print("RNG file:", cfg.rng_file)
    print("Sequence length:", cfg.sequence_length)
    print("Use bits:", cfg.use_bits)
    print("State window:", cfg.state_window)
    print("n_bins:", cfg.n_bins)
    ...
    print("\n[Phase3-RNG] Loading RNG dataset...")

    df_full = load_rng(
        rng_file=cfg.rng_file,
        sequence_length=cfg.sequence_length,
        state_window=cfg.state_window,
        use_bits=cfg.use_bits,
        verbose=cfg.verbose,
    )

    state_cols = [f"x{i}" for i in range(cfg.state_window)]
    df = df_full[state_cols]

    print(f"[Phase3-RNG] Observations: {len(df)}")
    print(f"[Phase3-RNG] Columns: {list(df.columns)}")

    if len(df) < 10:
        raise RuntimeError("Too few RNG windows loaded for Phase III RNG")
    ...
    print(
        f"[Phase3-RNG] Window split: train={len(idx_train_epochs)} | test={len(idx_test_epochs)}"
    )
    ...
    print("\n[Phase3-RNG] Discretizing RNG variables...")
    ...
    print("[Phase3-RNG] Discretization completed")
    ...
    if len(curr_all) < 10:
        raise RuntimeError("Too few transitions after RNG state encoding")

    print(f"[Phase3-RNG] Total transitions: {len(curr_all)}")
    ...
    print(
        f"[Phase3-RNG] Leakage-safe transitions: "
    ...
    print("\n[Phase3-RNG] Building empirical baseline kernel P0...")
    ...
    print("\n[Phase3-RNG] Estimating epsilon (train)...")
    ...
    print(f"[Phase3-RNG] eps_hat_train = {eps_hat_train:.4f}")

    # -----------------------------------------------------
    # Estimate epsilon (test)
    # -----------------------------------------------------

    print("\n[Phase3-RNG] Estimating epsilon (test)...")
    ...
    print(f"[Phase3-RNG] eps_hat_test = {eps_hat_test:.4f}")

    # -----------------------------------------------------
    # F1 — Injection recovery
    # -----------------------------------------------------

    print("\n[Phase3-RNG] Running Gate F1 (injection recovery)...")
    ...
    print("\n[Phase3-RNG] Running Gate F2 (controls collapse)...")

    eps_controls = run_rng_controls(
        curr=curr_train,
        nxt=nxt_train,
        kernel=P0,
        cfg=cfg,
    ...
    print("\n[Phase3-RNG] Running Gate F3 (holdout generalization)...")
    ...
    if comps_f3.shape[1] != enc.n_components:
        raise RuntimeError("F3 discretization changed number of RNG components")
    ...
    print(
        f"[Phase3-RNG] F3 eps_train = {eps_hat_train_f3:.4f} | "
    ...
    print("\n[Phase3-RNG] Running Gate F5 (sensitivity)...")
    ...
    if comps_alt.shape[1] != enc.n_components:
        raise RuntimeError("Sensitivity discretization changed number of RNG components")
    ...
    results = {
        ...
        "gates": summary,
        "metadata": {
            "use_bits": bool(cfg.use_bits),
            "state_window": int(cfg.state_window),
            "state_columns": state_cols,
            "n_bins": int(cfg.n_bins),
            "train_ratio": float(cfg.train_ratio),
            "rng_file": str(cfg.rng_file),
    ...
    out_file = cfg.results_dir / "phase3_rng_results.json"
